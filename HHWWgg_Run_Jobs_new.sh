...
options=$(getopt -o gcvstwr) # end name with colon ':' to specify argument string
# options=$(getopt -o gcvstwr --long nEvents:,labelName:,json:,condorQueue:,year: -- "$@") # end name with colon ':' to specify argument string

# options=`getopt -o gcvstwr --long nEvents: --long labelName: --long json: --long condorQueue: --long year: -- "$@"`
