...
all_variables = var.abe_variables # add variable lists together 
...
cfgTools.addCategories(process.HHWWggCandidateDumper, # className, src, ... from _cff ^^ 
                        [
                            ...
                            #("SemiLeptonic","", 0),
                            #("FullyLeptonic","", 0)
                            #("3photons","phoVector.size() == 3", 0), # I think variable accessed from HHWWgg object.. (phoVector for example)
                            #("2photons","phoVector.size() == 2", 0) # Only plotting when there are exactly 2 photons 
