...
namespace flashgg {
  ...
  private:
...

    {
      ...
      MVA_2D_Cut = fs->make<TH2D> ("MVA_2D_Cut","MVA_2D_Cut",210,-1.,1.,210,-1.,1.);
      MVA_2D_Cut_lep_cut = fs->make<TH2D> ("MVA_2D_Cut_lep_cut","MVA_2D_Cut_lep_cut",210,-1.,1.,210,-1.,1.);
...
//-----------------------------------------------------------------------------------------------------------
      ...
      //chuw/for (unsigned int diphoton_idx = 0; diphoton_idx < diPhotonTokens_.size(); diphoton_idx++) { //looping over all diphoton systematics
        ...
        for( unsigned int diphoIndex = 0; diphoIndex < 1; diphoIndex++ ) { // only look at highest pt dipho
          ...
          if(passMVAs && dipho->mass()>100. && dipho->mass()<180.)
         ...
         // chuw//n_good_jets = tagJets.size();
          ...
          for(float i=0.0;i<=70.;i++){
            if(theMET->getCorPt() >= i && dipho->mass()<180. && dipho->mass()>100)
            num_MetPtCut->Fill(i);
          //if(theMET->getCorPt() >= MetPtThreshold_)
            //Cut_Variables[12] = 1.0;
          //else 
          ...
          if ( (n_good_leptons >=2 ) && (theMET->getCorPt() >= MetPtThreshold_) && num_FL_dr>=1 ){
            ...
            if (n_good_electrons >= 2){
              ...
              for(double i=0.;i<=min(50.,tag_electron2->pt());i++){
                ...
                if(tag_electron2->pt() >= i && tag_electron1->pt() >= j && dipho->mass()<180. && dipho->mass()>100)
                lepPt2DCut->Fill(i,j);
                }
              }
              }
              ...
              cout<<"MassT:"<<MassT<<"MassT_l2:"<<MassT_l2<<endl;
                if((tag_electron1->charge()*tag_electron2->charge()==-1) && tag_electron1->pt()>=lep1ptThre_ && tag_electron2->pt()>=lep1ptThre_  && DiEleLV.Pt()>DiLepPtThre_ && DiEleLV.M()>DiLepMassThre_ && MassT>MassTThre_ && MassT_l2 >MassT_l2Thre_ && Save == 1.){
            ...
            if (n_good_muons >= 2  ){
              ...
              for(double j=i;j<=min(tag_muon1->pt(),70.);j++){
                cout<<j<<endl;
if(tag_muon2->pt() >= i && tag_muon1->pt() >= j && dipho->mass()<180. && dipho->mass()>100)
  lepPt2DCut->Fill(i,j);
              }
              }
              }
              ...
              MassT_l2 = sqrt(2*Mu2LV.Pt()*theMET->pt()*(1-cos(abs(Mu2LV.Phi()-theMET->phi()))));
                if((tag_muon1->charge()*tag_muon2->charge() == -1) && tag_muon1->pt()>=lep1ptThre_ && tag_muon2->pt()>=lep2ptThre_  && DiMuLV.Pt()>DiLepPtThre_ && DiMuLV.M()>DiLepMassThre_ && MassT>MassTThre_ && MassT_l2>MassT_l2Thre_ && Save == 1.){
            ...
            reco::Candidate::LorentzVector MuLV = tag_muon1->p4();
              ...
              if(tag_electron1->pt() >= i && tag_muon1->pt() >= j && dipho->mass()<180. && dipho->mass()>100. ){
              lepPt2DCut->Fill(i,j);
              }
              }
              else{
              if(tag_muon1->pt() >= i && tag_electron1->pt() >= j && dipho->mass()<180. && dipho->mass()>100. ){
              lepPt2DCut->Fill(i,j);
              }
              }
              }
              ...
              if((((tag_electron1->pt()>=lep1ptThre_) && (tag_muon1->pt()>=lep2ptThre_))||((tag_muon1->pt()>=lep1ptThre_) && (tag_electron1->pt()>=lep2ptThre_))) && (tag_muon1->charge()*tag_electron1->charge()==-1) && (DiLepLV.M()>DiLepMassThre_) && (DiLepLV.Pt()>DiLepPtThre_) && (MassT_l2>MassTThre_) && (MassT>MassT_l2Thre_) && (Save==1.)){
