...
class SamplesManager(object):
    ...
    def getFilesFomEOS(self,dsetName):
        ...
        print "start"
        print dsetName
        #print self.parallel_
        #if not self.parallel_:
        #    self.parallel_ = Parallel(200,self.queue_,maxThreads=self.maxThreads_,asyncLsf=True)
        import os
        files = []
        fileList = os.listdir(dsetName)
        for file in fileList:
            #print "==> ",file
            if file.endswith(".root") and not "faild" in file:
                import ROOT as ROOT
                #files.append( {"name":file.replace("myM","root://eosuser.cern.ch//eos/user/r/rasharma/post_doc_ihep/double-higgs/ntuples/MicroAODFiles/"), "nevents":500} )
                tempFile = ROOT.TFile("root://eosuser.cern.ch/"+dsetName+os.sep+file)
                if not tempFile:
                    print '\n==>Failed to open %s' % ("root://eosuser.cern.ch/"+dsetName+os.sep+file)
                    print '\n'
                    exit(0)
                tree = tempFile.Get('Events')
                nevents_temp = tree.GetEntries()
                # print "root://eosuser.cern.ch/"+dsetName+os.sep+file,"\t Entries = ",nevents_temp
                files.append( {"name":"root://eosuser.cern.ch/"+dsetName+os.sep+file, "nevents":500} )
